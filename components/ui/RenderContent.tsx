      ...
      if (i + 2 < parts.length) {
        ...
        elements.push(
          <div key={`code-${i}`} className={`relative group ${isReceiver ? "my-6" : "my-3"}`}>
            ...
            <pre className={`p-4 pt-10 rounded-lg overflow-x-auto border border-gray-700 m-0 bg-[#1e1e1e] text-left ${isReceiver ? "shadow-2xl shadow-black/50 p-5" : "shadow-inner"}`}>
